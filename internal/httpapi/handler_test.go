...
func TestPOCRulesetBlocksInvalidExpectationRound2(t *testing.T) {
	recorder := httptest.NewRecorder()
	...
	if response.Result != deliberatelyWrongExpectedResult {
		t.Fatalf("POC round 2 intentional failure: result = %d, want %d", response.Result, deliberatelyWrongExpectedResult)
	}
}
